    ...
    def _try_move(self, command: MoveCommand):
        ...
        for piece in board_copy.pieces:
            if self._status == ChessGame.STATUS_WHITE_MOVE and \
                    board_copy.white_pawn1_position in piece.get_threatened_positions(board_copy):
                return False
            elif self._status == ChessGame.STATUS_BLACK_MOVE and \
                    board_copy.black_pawn1_position in piece.get_threatened_positions(board_copy):
                return False
        return True
